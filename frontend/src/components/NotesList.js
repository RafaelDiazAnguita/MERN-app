...
export default class NotesList extends Component {
  ...
  renderPage = () => {
    ...
    if(sessionStorage['logged']){
      ...
      <div className='row row-cols-3'>
        {
          this.state.notes.map(note => {
            if(note.author == sessionStorage['username'])
            ...
            <div className='custom-card py-3 px-4'>
              ...
              <div className='row'>
                <div className='col-md-6 date-col'><h6>{format(note.date)}</h6></div>
              </div>
      ...
      return <div className='p-5 m-5 bg-white text-center'>You're not logged</div>
    }
  }
